...
import { databases, APPWRITE_DATABASE_ID, APPWRITE_BARGAIN_REQUESTS_COLLECTION_ID, APPWRITE_MARKETPLACE_COLLECTION_ID } from '@/lib/appwrite';
...
export interface BargainRequest extends Models.Document {
  ...
  bargainPrice: string;
  message?: string;
  status: "pending" | "accepted" | "rejected" | "cancelled" | "denied"; // Added 'denied'
  buyerId: string;
  buyerName: string;
  sellerId: string;
  sellerName: string;
...
export const useBargainRequests = () => {
  ...
  const fetchBargainRequests = useCallback(async () => {
    ...
    try {
      const queries = [
        ...
        Query.or([
          Query.equal("buyerId", user?.$id), // Corrected from user?.id
          Query.equal("sellerId", user?.$id), // Corrected from user?.id
        ]),
      ];
  ...
  }, [userProfile?.collegeName, user?.$id]); // Corrected dependency
