...
export const useServiceListings = (categories: string[] = []) => {
  ...
  const fetchServices = useCallback(async () => {
    ...
    try {
      ...
      setServices(response.documents as unknown as ServicePost[]); // Explicit cast to unknown first
    } catch (err: any) {
    ...
    const unsubscribe = databases.client.subscribe(
      ...
      (response) => {
        if (response.events.includes("databases.*.collections.*.documents.*.create")) {
          setServices((prev) => [response.payload as unknown as ServicePost, ...prev]);
        } else if (response.events.includes("databases.*.collections.*.documents.*.update")) {
          setServices((prev) =>
            prev.map((service) =>
              service.$id === (response.payload as ServicePost).$id
                ? (response.payload as unknown as ServicePost)
                : service
            )
          );
