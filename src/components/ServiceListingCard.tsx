...
const ServiceListingCard: React.FC<ServiceListingCardProps> = ({ service }) => {
  ...
  const { reviews, isLoading: isReviewsLoading, error: reviewsError } = useServiceReviews(service.$id);

  const averageRating = reviews.length > 0
    ? (reviews.reduce((sum, review) => sum + review.rating, 0) / reviews.length).toFixed(1)
    : null;
  
  const hasReviewed = user ? reviews.some(review => review.reviewerId === user.$id) : false;
  ...
  return (
    <Card className="bg-card text-card-foreground shadow-md border-border">
      ...
      <CardContent className="p-4 pt-0 space-y-1">
        ...
        {averageRating && (
          <p className="text-xs text-muted-foreground">Rating: <span className="font-medium text-yellow-500">{averageRating} ★</span> ({reviews.length} reviews)</p>
        )}
