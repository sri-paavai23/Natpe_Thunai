...
const ServiceListingCard: React.FC<ServiceListingCardProps> = ({ service }) => {
  ...
  const handleBargainClick = () => {
    ...
    if (user.$id === service.providerId) { // Corrected from posterId
  ...
  return (
    <Card className="bg-card text-card-foreground shadow-md border-border">
      ...
      <CardContent className="p-4 pt-0 space-y-1">
        ...
        <p className="text-xs text-muted-foreground">Posted by: {service.providerName}</p> {/* Corrected from posterName */}
