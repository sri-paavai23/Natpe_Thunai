...
export type FoodOfferingPostData = z.infer<typeof formSchema>;

interface PostFoodOfferingFormProps {
  onSubmit: (data: FoodOfferingPostData) => Promise<void>;
  onCancel: () => void;
  categoryOptions: { value: string; label: string }[];
  initialCategory?: string; // Prop for initial category
}

const PostFoodOfferingForm: React.FC<PostFoodOfferingFormProps> = ({ onSubmit, onCancel, categoryOptions, initialCategory }) => {
  const [isSubmitting, setIsSubmitting] = useState(false);

  const form = useForm<FoodOfferingPostData>({
  ...
  const handleSubmit = async (data: FoodOfferingPostData) => {
    ...
    try {
      ...
      console.error("Error posting food offering:", error);
      toast.error(error.message || "Failed to post food offering.");
    } finally {
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel className="text-foreground">Offering Title</FormLabel>
              <FormControl>
        ...
        <DialogFooter className="pt-4 flex flex-col sm:flex-row gap-2">
          ...
          <Button type="submit" disabled={isSubmitting} className="w-full sm:w-auto bg-secondary-neon text-primary-foreground hover:bg-secondary-neon/90">
            {isSubmitting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <><PlusCircle className="mr-2 h-4 w-4" /> Post Offering</>}
          </Button>
        </DialogFooter>
      </form>
    </Form>
...
export default PostFoodOfferingForm;